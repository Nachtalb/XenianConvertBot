...
from telegram.ext import CommandHandler, Filters, Handler, MessageHandler, Updater, messagequeue
    ...
    def add_command(self, handler: Type[Handler] or Handler = None, name: str = None, func: Callable = None, **kwargs):
        ...
        if isinstance(handler, Handler):
            ...
            if handler == MessageHandler:
                self.updater.dispatcher.add_handler(handler=handler(callback=func, **kwargs))
            else:
...
def main():
    global converter_bot
    converter_bot = ConverterBot(TELEGRAM_API_TOKEN, mode=MODE['active'], mode_config=MODE.get('configuration'),
                                 admins=ADMINS)
    converter_bot.start()
